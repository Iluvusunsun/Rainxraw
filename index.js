...
// ─── DATABASE (JSON file-based, no native modules) ────────────────
const DB_DIR = process.env.DB_PATH || path.join(__dirname, 'data');
const DB_FILE = path.join(DB_DIR, 'rainx.json');
...
app.get('/api/raw/:id', (req, res) => {
  const db = loadDB();
  const paste = checkExpired(db[req.params.id]);
  ...
  const db2 = loadDB(); db2[paste.id] = paste; saveDB(db2);
...
app.post('/api/paste', requireKey, (req, res) => {
  ...
  const paste = { id, name, lang, content, visibility, expire, expire_at: ms ? created_at + ms : null, views: 0, created_at };
  const db = loadDB();
  db[id] = paste;
  saveDB(db);
...
<div class="container">
  ...
  <div class="panel active" id="panel-new">
    <div class="row">
      <div class="form-group"><label>Name</label><input type="text" id="paste-name" placeholder="script.lua"></div>
      <div class="form-group"><label>Language</label><select id="paste-lang"><option value="plaintext">Plaintext</option><option value="lua">Lua</option><option value="javascript">JavaScript</option><option value="python">Python</option></select></div>
    </div>
    <div class="row">
      <div class="form-group"><label>Expire</label><select id="paste-expire"><option value="never">Never</option><option value="1h">1 Hour</option><option value="24h">24 Hours</option><option value="7d">7 Days</option></select></div>
      <div class="form-group"><label>Visibility</label><select id="paste-vis"><option value="public">Public</option><option value="private">Private</option></select></div>
    </div>
    <div class="form-group"><label>Content</label><textarea id="paste-code" placeholder="-- paste code here..."></textarea></div>
    <div style="display:flex;gap:8px">
      <button class="btn btn-primary" id="create-btn" onclick="createPaste()">STORE</button>
      <button class="btn btn-ghost" onclick="clearForm()">CLEAR</button>
    </div>
    <div class="result-box" id="result-box">
      <div class="result-header"><span class="result-title">✓ STORED</span><button class="btn btn-outline" onclick="copyUrl()" style="font-size:9px;padding:4px 12px">COPY URL</button></div>
  ...
  <div class="modal">
    ...
    <div class="modal-body">
      <div class="guard-overlay active" id="guard-overlay" onclick="revealCode()">
        <div class="guard-ring"></div><div class="guard-ring"></div><div class="guard-ring"></div>
...
function switchTab(n){document.querySelectorAll('.tab').forEach((t,i)=>t.classList.toggle('active',['new','list'][i]===n));document.querySelectorAll('.panel').forEach(p=>p.classList.remove('active'));$('panel-'+n).classList.add('active');if(n==='list')loadList();}
async function createPaste(){if(!key()){toast('⚠ กรอก API Key ก่อน');return;}const content=$('paste-code').value;if(!content.trim()){toast('⚠ กรอก content ก่อน');return;}const btn=$('create-btn');btn.disabled=true;btn.textContent='STORING...';try{const r=await fetch('/api/paste',{method:'POST',headers:{'Content-Type':'application/json','Authorization':'Bearer '+key()},body:JSON.stringify({name:$('paste-name').value.trim()||'untitled',lang:$('paste-lang').value,content,expire:$('paste-expire').value,visibility:$('paste-vis').value})});const d=await r.json();if(!r.ok)throw new Error(d.error||'Failed');$('result-box').classList.add('show');$('result-url').textContent=base()+d.url;$('result-raw').textContent='Raw: '+base()+d.raw_url;toast('✓ STORED');}catch(e){toast('✗ '+e.message);}finally{btn.disabled=false;btn.textContent='STORE';}}
function clearForm(){$('paste-name').value='';$('paste-code').value='';$('result-box').classList.remove('show');}
function copyUrl(){navigator.clipboard.writeText($('result-url').textContent).then(()=>toast('✓ COPIED'));}
async function loadList(){if(!key()){$('paste-list').innerHTML='<div class="empty"><div class="empty-icon">🔑</div>กรอก API Key ก่อน</div>';return;}$('paste-list').innerHTML='<div class="loading">LOADING...</div>';try{const r=await fetch('/api/list',{headers:{'Authorization':'Bearer '+key()}});const d=await r.json();if(!r.ok)throw new Error(d.error);$('s-total').textContent=d.total;$('s-pub').textContent=d.pastes.filter(p=>p.visibility==='public').length;$('s-prv').textContent=d.pastes.filter(p=>p.visibility==='private').length;if(!d.pastes.length){$('paste-list').innerHTML='<div class="empty"><div class="empty-icon">📭</div>NO PASTES YET</div>';return;}$('paste-list').innerHTML=d.pastes.map(p=>\`<div class="paste-item" onclick="openPaste('\${p.id}')"><div class="paste-info"><div class="paste-name">\${p.name}</div><div class="paste-meta"><span>\${p.lang}</span><span>👁 \${p.views}</span><span>\${p.visibility==='private'?'🔒':'🌐'}</span><span>\${p.id}</span></div></div><div class="paste-actions" onclick="event.stopPropagation()"><button class="icon-btn" onclick="copyRaw('\${p.id}')">RAW</button><button class="icon-btn del" onclick="delPaste('\${p.id}')">✕</button></div></div>\`).join('');}catch(e){$('paste-list').innerHTML=\`<div class="empty"><div class="empty-icon">⚠</div>\${e.message}</div>\`;}}
async function delPaste(id){if(!confirm('ลบ paste นี้?'))return;await fetch('/api/paste/'+id,{method:'DELETE',headers:{'Authorization':'Bearer '+key()}});toast('✓ DELETED');loadList();}
function copyRaw(id){navigator.clipboard.writeText(base()+'/api/raw/'+id).then(()=>toast('✓ RAW COPIED'));}
async function openPaste(id){guardActive=true;$('guard-overlay').classList.add('active');$('modal-code').classList.add('guarded');$('guard-btn').textContent='👁';$('modal-title').textContent='Loading...';$('modal-code').textContent='';$('modal-overlay').classList.add('show');try{const h=key()?{'Authorization':'Bearer '+key()}:{};const[mr,rr]=await Promise.all([fetch('/api/paste/'+id,{headers:h}),fetch('/api/raw/'+id,{headers:h})]);const meta=await mr.json();const code=await rr.text();$('modal-title').textContent=meta.name||id;$('modal-code').textContent=code;}catch(e){$('modal-code').textContent='Error: '+e.message;}}
function revealCode(){guardActive=false;$('guard-overlay').classList.remove('active');$('modal-code').classList.remove('guarded');$('guard-btn').textContent='🛡';}
function toggleGuard(){if(guardActive)revealCode();else{guardActive=true;$('guard-overlay').classList.add('active');$('modal-code').classList.add('guarded');$('guard-btn').textContent='👁';}}
function closeModal(){$('modal-overlay').classList.remove('show');}
function copyCode(){navigator.clipboard.writeText($('modal-code').textContent).then(()=>toast('✓ COPIED'));}
